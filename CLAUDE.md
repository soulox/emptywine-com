...
- Non-secret SMTP settings (`SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURITY`, `SMTP_USER`, `SMTP_FROM`) are `vars` in `wrangler.json` (committed). Secrets (via `wrangler secret put`): `ADMIN_TOKEN`, `SMTP_PASSWORD`, optional `SLACK_WEBHOOK_URL`. Local dev reads secrets from `.dev.vars` (gitignored; holds `ADMIN_TOKEN` + `SMTP_PASSWORD`)
